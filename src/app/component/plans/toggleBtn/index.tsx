...
const Index: FC<ToggleBtnProps> = ({ planType, onChange }) => {
  ...
  return (
    <div className=" min-w-[350px] h-14">
      <label
        htmlFor="Toggle3"
        className="inline-flex gap-4  bg-cloudWhite items-center justify-center cursor-pointer dark:text-gray-800 rounded-lg w-full h-full p-1"
      >
        <input
          id="Toggle3"
          type="checkbox"
        ...
        />
        <span
          className={`px-4 py-2 rounded-lg ${planType === 'Individual' ? 'bg-lavenderPurple text-white' : ''
        ...
        <span
          className={`px-4 py-2 rounded-lg ${planType === 'Enterprise' ? 'bg-lavenderPurple text-white' : ''
            } bg-cloudWhite flex-1 text-center h-full grid place-items-center ${quaternaryHeading} font-medium 
`}
        >
          Enterprise Plan
