...
export function ProjectRFICenter({ projectId }: Props) {
  ...
  const authUser = useSelector((state: RootState) => state.auth.user as { user?: IUser });
  ...
  return (
    <div className="mb-4 md:ms-[69px] md:me-[59px] mx-4  ">
      ...
      {parentRfis.filter(rfi => {
        ...
        return <div key={rfi._id} className='px-2 bg-white rounded-lg shadow mt-4 space-y-2'>
          <div className="flex">
            <Avatar
            ...
            <div className="py-4 flex-1 px-4 space-y-2">
              <div className="flex justify-between">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  {typeof rfi.user !== 'string' && rfi.user._id === authUser.user?._id ? <UpdateRFI onSuccess={(_rfi) => {
                    ...
                    setRfis(updatedRfis);
                  }} rfiData={rfi} projectId={projectId} /> : null}
                </div>
              </div>
              <TertiaryHeading
              ...
              {rfi.file ? <div className='mt-2 p-2 flex items-center border border-[#E1E5EA] rounded-md w-[270px] space-x-2'>
          ...
          {/* replies */}
          {rfiReplies(rfi._id).map(reply => <div key={reply._id} className='flex border-t'>
            <Avatar
              src={typeof reply.user !== 'string' ? reply.user.avatar ?? reply.user.companyLogo : ""}
              size={40}
              className='mt-4'
            />
            ...
            <div className="py-4 flex-1 px-4 space-y-2">
              <div className="flex justify-between">
                ...
                />
                <div className="flex items-center space-x-4">
                  {reply.type === 'private' ? <p className="py-[5px] rounded-full px-[11px] bg-[#E9EBF8] text-[#7138DF] text-xs leading-4">
                    Private
                  </p> : null}

                  {typeof reply.user !== 'string' && reply.user._id === authUser.user?._id ? <UpdateRFI onSuccess={(_rfi) => {
                    const updatedRfis = [...rfis].map(rfiItem => {
                      if (rfiItem._id === _rfi._id) {
                        return _rfi;
                      }
                    ...
                    setRfis(updatedRfis);
                  }} rfiData={reply} projectId={projectId} /> : null}
                </div>
              </div>
              <TertiaryHeading
              ...
              {reply.file ? <div className='mt-2 p-2 flex items-center border border-[#E1E5EA] rounded-md w-[270px] space-x-2'>
                <div className='w-[44px] h-[44px] justify-center rounded-md items-center flex bg-[#FFEBC6]' >
                  <Avatar
                    src={"/file-yellow.svg"}
                    alt='file icon'
