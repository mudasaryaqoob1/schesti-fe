...
export function ProjectIntro({ id }: Props) {
    ...
    const query = useQuery(['getOwnerProjectById', id], () => {
        return bidManagementService.httpGetOwnerProjectById(id);
    },
        {
            onSuccess(data) {
                if (data.data && data.data.project) {
                    dispatch(bidManagementOwnerActions.setProjectAction(data.data.project));
                }
            },
            onError(error) {
    ...
    return <div className="flex justify-between items-center">
        <div className="space-y-3">
            ...
            <div className="flex space-x-4 items-center text-[#667085] text-base leading-6 font-normal">
                <SenaryHeading title={`Creation Date: ${bid ? momentTimezone(bid.createdAt).format("MM/DD/YYYY hh:mm A") : ""}`} />
                <SenaryHeading title={`Bid Date: ${bid ? momentTimezone(bid.bidDueDate).format("MM/DD/YYYY hh:mm A") + " " + getTimezoneFromCountryAndState(bid.country, bid.state) : ""}  `} />
            </div>
        </div>
