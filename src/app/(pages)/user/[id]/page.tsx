...
const UserProfile = () => {
    const { id } = useParams();
    const [userData, setUserData] = useState<IUserInterface>({} as IUserInterface);
    ...
    return (
        <section className="my-4 mx-8 px-4 gap-6">
            ...
            <Intro name={userData.socialName || userData.name} avatar={userData.socialAvatar || userData.avatar} fetchUser={() => setFetchUser(prev => !prev)} />
            <UserPosts />
        </section>
    )
}
