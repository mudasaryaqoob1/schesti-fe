...
import { selectSupportTickets, selectSupportTicketsLoading } from '@/redux/company/companySelector'
import { deleteSupportTicket, fetchSupportTickets } from '@/redux/company/company.thunk'
...
const SupportTickets = () => {
    ...
    const handleDropdownItemClick = async (key: string, supportTicket: ISupportTicket) => {
        if (key == 'delete') {
    ...
    return (
        <CustomNavbar>
            <section className='px-16 mt-6'>
                ...
                {
                    supportTicketsLoading ? <h6>Loading...</h6> : supportTicketsData?.map((supportTicket: ISupportTicket, i: number) => {
                        const { _id, createdAt, title, description, updatedAt } = supportTicket;
                        return (
                            <div key={i} className='shadow-primaryGlow rounded-2xl cursor-pointer flex flex-col p-4 mt-6'>
                                <div className='flex items-center justify-between'>
                                    <div className='flex gap-4'>
                                        <Description title={`Ticket # ${_id}`} className='text-steelGray font-semibold' />
                                        <p className='bg-lightblue rounded-xl px-2'>
                                            <span className='text-xs text-darkblue font-normal'>In Progress</span>
                                        </p>
                                    </div>
                                    <div className='flex items-center gap-2'>
                                        <Description title='4 seconds ago' className='text-lightenGreyish' />
                                        <Dropdown
                                            menu={{
                                                items,
                                                onClick: (event) => {
                                        ...
                                        >
                                            <Image src='/more-horizontal.svg' width={24} height={6} alt='options' className='cursor-pointer' />
                                        </Dropdown>
                                    </div>
                                </div>
                                <QuinaryHeading title={title} className='text-base font-medium mt-3' />
                                <Description className='text-steelGray mt-2'
                                    title={description} />
                                <div className='flex items-center gap-7 mt-5'>
                                    ...
                                    <p className='text-xs text-slateGray font-normal flex gap-1 items-center'>
                                        <Image src='/reply-rounded.svg' alt='date' width={16} height={14} />
