                    ...
                    <div className="container mx-auto px-6">
                        ...
                        <div>
                            <Tabs value={activeTab}>
                                <TabsHeader className="rounded-none bg-transparent p-0 space-x-4" indicatorProps={{
                                    className: "bg-transparent-2 border-gray-900 shadow-none rounded-none",
                                }}
                                    placeholder={undefined} onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>
                                    {tabsData.map((tab, index) => (
                                        <Tab key={index} value={tab.value !== undefined ? tab.value?.toString() : ""} onClick={() =>
                                            setActiveTab(tab.value !== undefined ? tab.value.toString() : "")} className={activeTab
                                                === tab.value ? "whitespace-nowrap width-xl height-xl foundation-primary-700" :
                                                "whitespace-nowrap width-xl height-xl"} placeholder={undefined}
                                            onPointerEnterCapture={undefined} onPointerLeaveCapture={undefined}>{tab.label}</Tab>
                                    ))}
                                </TabsHeader>
                                <TabsBody placeholder={undefined} onPointerEnterCapture={undefined}
                                    onPointerLeaveCapture={undefined} className='hidden'>
                                    {tabsData.map((tab, index) => (
                                        <TabPanel key={index} value={tab.value !== undefined ? tab.value : ""}>{tab.label}
                                        </TabPanel>
                                    ))}
                                </TabsBody>
                            </Tabs>
                                        ...
                                        <>
                                            ...
                                            <div className='lg:col-span-5 mt-5'>
                                                {convenienceStore.map((item, index) => (
                                                    <div key={index} className='bg-white rounded-lg cosmicGray mb-5 px-3.5'>
                                                        <MessageComponent
                                                            title={item.title}
                                                            posted={item.posted}
