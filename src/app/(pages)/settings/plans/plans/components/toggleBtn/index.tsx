...
const Index: FC<ToggleBtnProps> = ({ planType, onChange }) => {
  ...
  return (
    <div className=" min-w-[350px] h-14">
      <label
        htmlFor="Toggle3"
        className="inline-flex gap-4  bg-schestiLightPrimary items-center justify-center cursor-pointer dark:text-gray-800 rounded-full w-full h-full p-1"
      >
        <input
          id="Toggle3"
          type="checkbox"
        ...
        />
        <span
          className={`px-4 py-2 rounded-full ${planType === 'Individual' ? 'bg-schestiPrimary text-white' : ''
        ...
        <span
          className={`px-4 py-2 rounded-full ${planType === 'Enterprise' ? 'bg-schestiPrimary text-white' : ''
            } bg-schestiLightPrimary flex-1 text-center h-full grid place-items-center ${quaternaryHeading} font-medium 
`}
        >
          Enterprise Plan
