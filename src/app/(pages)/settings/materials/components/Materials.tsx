...
import { reduxMaterialsData, reduxMaterialsLoading } from '@/redux/company/settingSlices/settingSelector';
...
const Materials = () => {
    ...
    const [materialUploadingError, setMaterialUploadingError] = useState<any>(false);
    ...
    const uploadMaterialsHandler = async (e: any, category: string, resetCategory: () => void) => {
        ...
        try {
            ...
            const { statusCode } = await materialService.httpUploadMaterialsData(formData);
    ...
    const handleUpdateMaterial = async ({ unitEquipments, unitLabourHour, unitMaterialCost }: InitialValuesTypes) => {
        const { statusCode, data } = await materialService.httpUpdateMaterial(selectedRowId, { unitEquipments, unitLabourHour, unitMaterialCost });
                            ...
                            >
                                <div className="flex justify-between items-center gap-4">
                                    ...
                                    <div className="flex gap-4 items-center">
                                        ...
                                        <div>
                                            {
                                                isUploadingMaterials ? <p>Uploading...</p> : (
                                                    <div>
                                                        <label
                                                        ...
                                                        <input
                                                            ...
                                                            onChange={(e: any) => {
                                                                uploadMaterialsHandler(e, values.category, () => setFieldValue('category', ''));
                                                            }
                                ...
                                <div>
                                    <div className="grid grid-cols-8 bg-graphiteGray p-3 rounded-lg mt-7 bg-opacity-10">
                                        ...
                                        <Description title='Unit Labor Hour' className='col-span-1' />
                                        <Description title='Unit material cost' className='col-span-1' />
                                        <Description title="Unit equipment's" className='col-span-1' />
                                        <Description title="Actions" className='col-span-1 text-center' />
                                    </div>
                                    <div className='gap-4'>

                                        {
                                            materialsLoading ? <div className='mt-4'><Skeleton /> </div> : materialsData?.length < 1 ? <TertiaryHeading className='mt-4 text-center' title='No Data Available' /> : materialsData?.map(({ _id: category, subcategories }: any, i: number) => {
                                                return (
                                                    <div key={i}>
                                                        <div className="flex items-center gap-3 mt-4" >
                                                            <QuaternaryHeading title={category} className="font-bold capitalize" />
                                                        ...
                                                        {
                                                            subcategories?.map(({ _id, subCategory, description, unit, unitLabourHour, unitMaterialCost, unitEquipments }: any) => (
                                                                <div key={_id} className='mt-3'>
                                                                    <div className="grid grid-cols-8 gap-2 border border-b-graphiteGray border-transparent p-3 mt-3 border-opacity-20 bg-opacity-10">
                                                                        <Description title={subCategory} className='col-span-1' />
                                                                        <Description title={description} className='col-span-2' />
                                                                        <Description title={unit} className='col-span-1' />
                                                                        <div className='col-span-1'>
                                                                            <FormControl
                                                                                control="simpleInput"
                                                                                type="text"
                                                                                value={_id === selectedRowId ? values.unitLabourHour : unitLabourHour}
                                                                        ...
                                                                        <div className='col-span-1'>
                                                                            <FormControl
                                                                                control="simpleInput"
                                                                                type="text"
                                                                                value={_id === selectedRowId ? values.unitMaterialCost : unitMaterialCost}
                                                                                disabled={_id !== selectedRowId}
                                                                        ...
                                                                        <div className='col-span-1 flex gap-2'>
                                                                            <FormControl
                                                                                control="simpleInput"
                                                                                type="text"
                                                                                value={_id === selectedRowId ? values.unitEquipments : unitEquipments}
                                                                                disabled={_id !== selectedRowId}
                                                                        ...
                                                                        <div className='col-span-1 flex justify-center items-center'>
                                                                            {
                                                                                _id === selectedRowId ? <button type='submit'>
                                                                                    <Image
                                                                                        src={_id === selectedRowId ? '/hand-drawn-tick.svg' : "/edit.svg"}
                                                                                        ...
                                                                                        onClick={
                                                                                            () => {
                                                                                                if (_id !== selectedRowId) {
                                                                                                    setFieldValue('unitMaterialCost', unitMaterialCost);
                                                                                                    setFieldValue('unitLabourHour', unitLabourHour);
                                                                                                    setFieldValue('unitEquipments', unitEquipments);
                                                                            ...
                                                                            <Image
                                                                                ...
                                                                                onClick={
                                                                                    async () => {
                                                                                        const { statusCode } = await materialService.httpDeleteMaterial(_id);
                                                                                        if (statusCode === 200) {
                                                                                            fetchMaterialsData()
