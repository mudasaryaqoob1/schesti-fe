...
export function UploadSubCategoriesModal({ open, setOpen, categories }: Props) {
    ...
    const categoryOptions = categories.map(category => {
        return {
            label: <p className="text-[14px] text-black font-normal space-x-2 leading-3">
    ...
    async function handleUpload(file: RcFile) {
        ...
        try {
            ...
            const response = await categoriesService.httpParseSubCategoriesCSV(formData);
            if (response.data) {
            ...
            const errMessage = err.response?.data.message || 'An error occurred while uploading the file';
            setError(errMessage);
        } finally {
