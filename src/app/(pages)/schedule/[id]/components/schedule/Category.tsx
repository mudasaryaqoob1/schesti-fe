...
type Props = {
    ...
    handleWbs(_category: IWBSType['category'], _subCategory: IWBSType['subCategory']): void;
}
export function CategoryModal({ materialModal, setMaterialModal, handleWbs }: Props) {
    ...
    function handleCreateWbs() {
        ...
        const c = categories.find((cat: { value: string }) => cat.value === category);
        const s = subCategories.find((cat: { value: string }) => cat.value === subCategory);
        if (!c || !s) {
            return;
        }
        handleWbs(c, s);
    ...
    return <CustomModal setOpen={() => setMaterialModal(false)} open={materialModal}>
