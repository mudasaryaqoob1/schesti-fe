...
const SearchFilters = () => {
    ...
    const getAllTrades = async () => {
        try {
            const { data: { trades } }: any = await tradeService.httpGetAllTrades();
            setTrades(trades);
                                ...
                                <div className='flex flex-wrap gap-1 border-b border-lightSteelGray pb-6'>
                                    {selectedStates.map((state, i) => (
                                        <div key={i} className="flex gap-0.5 rounded-[163px] bg-schestiLightPrimary items-center px-2 py-1.5">
                                            ...
                                            <Image onClick={() => {
                                                dispatch(setSelectedStates(selectedStates.filter(currentState => currentState !== state)))
                                            }} src='/crossblack.svg' className='cursor-pointer' alt='close' width={10} height={10} />
                                        </div>
                                    ))}
                            ...
                            <div className='mt-5'>
                                ...
                                <div className='mt-2 flex flex-wrap gap-1'>
                                    {selectedTrades.map((trade, i) => (
                                        <div key={i} className="flex gap-0.5 rounded-[163px] bg-schestiLightPrimary items-center px-2 py-1.5">
                                            <p className='text-xs text-graphiteGray'>{trades.find(({ _id }) => _id === trade)?.name}</p>
                                            <Image onClick={() => dispatch(setSelectedTrades(selectedTrades.filter(currentTrade => currentTrade !== trade)))} src='/crossblack.svg' className='cursor-pointer' alt='close' width={10} height={10} />
                                        </div>
                                    ))}
