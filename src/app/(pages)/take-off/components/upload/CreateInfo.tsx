...
const CreateInfo = () => {
  ...
  const getTakeOffDetails = async (id: string) => {
    try {
      ...
      let emptoAdd =
        Array.isArray(takeoff?.assignedUsers) &&
        Array.isArray(emp) &&
        emp.length > 0
      ...
      console.log(
        ...
        emptoAdd,
        pdata,
        ' ===> Data coming for single record of summaruy'
      );
                ...
                <div className="flex flex-col gap-y-7 w-[50%]">
                  ...
                  <ul className="list-none flex flex-col gap-y-5">
                    {selectedFiles &&
                      Array.isArray(selectedFiles) &&
                      selectedFiles?.length > 0 &&
                      selectedFiles?.map((it: any, ind: number) => {
                        // const totalProgress = fullData?.pages?.filter((i: any) => { return i?.fileId == it?.fileId })
                        return (
                          <li
                            key={ind}
                          ...
                          >
                            ...
                            {/* <Progress percent={(totalProgress && Array.isArray(totalProgress) ? Math.ceil((totalProgress?.length / it?.totalPages) * 100) : 0)} strokeColor={'#007AB6'} /> */}
                            {it?.fileId && (
                              <Progress percent={100} strokeColor={'#007AB6'} />
                            )}
