...
const CreateInfo = () => {
      ...
      render: (val, record) => {
        return (
          <div
            ...
            onClick={() => {
              setselectedAssignedUsers((ps: any) =>
                ps?.filter((i: any) => i != record)
              );
            }}
          >
  ...
  const makeApiCall = async () => {
    try {
      ...
      if (
        ...
        Array.isArray(selectedAssignedUsers) &&
  ...
  const addUserHandler = async (values: IUser) => {
    try {
      setisLoading(true);
      await userService.httpAddNewEmployee({
        ...values,
        roles: [values.roles],
      });
    ...
    <>
      ...
      <div className="flex justify-between mt-5">
        <div className="flex gap-4 items-center mt-6">
          ...
          <SenaryHeading
            title="Takeoff"
            className="font-base !font-semibold text-slateGray"
          />
          <Image
            src={'/chevron-right.svg'}
            alt="chevron-right icon"
      ...
      >
        {/* Project Info */}
        <div className="p-5 flex flex-col rounded-2xl border border-silverGray shadow-secondaryShadow bg-white flex-1">
          <TertiaryHeading
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 grid-rows-3 gap-x-4 gap-y-4">
                      ...
                      <FormControl
                        ...
                        handleChange={(val: any) => {
                          console.log(val);
                          setprojectData((ps: any) => ({ ...ps, status: val }));
                        }}
                        options={[
                          ...
                          {
                            label: 'Project Declined',
                            value: 'Project Declined',
                          },
                          {
                            label: 'Project Requires Approval',
                            value: 'Project Requires Approval',
                          },
                        ]}
                      />
                      <FormControl
        ...
        <div className="p-5 flex flex-col rounded-2xl border border-silverGray shadow-secondaryShadow bg-white flex-1">
          ...
          <div className="px-2">
            <Select
              ...
              value={
                selectedAssignedUsers?.map((record: any) => ({
                  label: record?.firstName ?? record?.email,
                  value: JSON.stringify(record),
                })) ?? []
              }
              onChange={(v: any) => {
                setselectedAssignedUsers(v?.map((su: any) => JSON.parse(su)));
              }}
              options={assignedUser?.map((record: any) => ({
                label: record?.firstName ?? record?.email,
                value: JSON.stringify(record),
              }))}
            />
                ...
                <div className="flex flex-col gap-y-7 w-[50%]">
                  {fullData?.files &&
                    ...
                    fullData?.files?.length > 0 && (
                      <h4 className="text-gray-600">
                        {fullData?.files?.length} uploaded files
                      </h4>
                    )}
                  <ul className="list-none flex flex-col gap-y-5">
                    {fullData?.files &&
                      Array.isArray(fullData?.files) &&
                      fullData?.files?.length > 0 &&
                      fullData?.files?.map((it: any, ind: number) => {
                        const totalProgress = fullData?.pages?.filter(
                          (i: any) => {
                            ...
                            <Progress
                              percent={
                                totalProgress && Array.isArray(totalProgress)
                                  ? Math.ceil(
                                      (totalProgress?.length / it?.totalPages) *
                                        100
                                    )
                                  : 0
                              }
                ...
                <input
                  ...
                  onChange={(e: any) => {
                    ...
                    if (e.target.files?.length > 0) {
                      const arr = Object.keys(e.target?.files)?.map(
                        (it: any) => {
