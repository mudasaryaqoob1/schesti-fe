...
const EditableCurvedShape: React.FC<EditableCurvedShapeProps> = ({ cur, id, scaleUnits, selectedShape, draw, setDraw, setSelectedShape, scale }) => {
  ...
  useEffect(() => {
    if (cur?.points) { setPoints(cur?.points) }
    if(cur?.controlPoints){setControlPoints(cur?.controlPoints)}
  }, [cur])
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < points.length; i += 2) {
      ...
      newControlPoints.push({ x: midX, y: midY, index: i, offsetX: 0, offsetY: 0 });
    }
    if(!(Array.isArray(cur?.controlPoints))) {setControlPoints(newControlPoints);}
  ...
  const handleControlPointDragMove = (e: any, controlIndex: number) => {
    ...
    const point1 = { x: points[controlPoint.index], y: points[controlPoint.index + 1] };
    const point2 = { x: points[(controlPoint.index + 2) % points.length], y: points[(controlPoint.index + 3) % points.length] };

    // Calculate offset from closest point on line
    const closestPoint = getClosestPointOnLine(point1, point2, { x: e.target.x(), y: e.target.y() });
  ...
  const getClosestPointOnLine = (A: { x: number, y: number }, B: { x: number, y: number }, P: { x: number, y: number }) => {
  ...
  const dragEnd = () => {
    ...
    const updatedDraw = {
      ...
      curve: draw.curve.map((line:any, index:number) =>
        index === +shapeNumber ? { ...line, points: points, controlPoints } : line
      ),
    };
    setDraw(updatedDraw)
  }

  const approximateCurve = (points: number[], controlPoints: ControlPoint[], numSegments: number): number[] => {
    ...
    for (let i = 0; i < points.length; i += 2) {
      ...
      const x0 = points[i];
      const y0 = points[i + 1];
      const x1 = controlPoints[controlIndex].x + controlPoints[controlIndex].offsetX;
      const y1 = controlPoints[controlIndex].y + controlPoints[controlIndex].offsetY;
